...
COURSERA_TOKEN = "iR9gYNyceu8xjGls"# the key provided to the Student under his/her email on submission page
COURSERA_EMAIL = "medha.shaharkar@ubs.com"# the email
...
for i in range (len(X)):
    ...
    for j in range (len(X[i])):
        sq_diff = (X[i][j]-Y[i][j])*(X[i][j]-Y[i][j])
        sum_sq_diff = sum_sq_diff+sq_diff
        euc_dist = math.sqrt(sum_sq_diff)
    euclidean_distances_vector_l.append(euc_dist)
### END CODE HERE ###
...
for i in range(len(X)):
    sum = 0
    sum = np.sum(sq_diff[i])
    euclidean_distances_vector_l_vectorized.append(np.sqrt(sum))
...
def VectorizationMethod(dim_space, sample_size, lower_boundary, upper_boundary, bins_number=30):
    ...
    for i in range(len(X)):
        sum = 0
        sum = np.sum(sq_diff[i])
        euclidean_distances_vector_v.append(np.sqrt(sum))
    ### END CODE HERE ###
